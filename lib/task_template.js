...
function TaskTemplate(queue, ...args) {
  ...
  // Fields for `.toObject()/.fromObject()`
  this.__serializableFields = [
    'name',
    'id',
    'pool',
    'uid',
...
//
TaskTemplate.prototype.run = async function () {
  await this;
  return this.queue.__addTask__(this);
};


// Postpone task to `delay` ms.
//
TaskTemplate.prototype.postpone = async function (delay = this.postponeDelay) {
  await this;
  return this.queue.__addTask__(this, delay);
};


// Force restart of a running task
//
TaskTemplate.prototype.restart = async function (add_retry, delay) {
  ...
  await this;
  return this.queue.__restartTask__(this, add_retry, delay);
};
...
TaskTemplate.prototype.progressAdd = async function (value) {
  ...
  if (success) {
    let progress = await this.queue.__redis__.hget(`${prefix}${this.id}`, 'progress');
...
//
TaskTemplate.prototype.handleCommand = async function (command) {
...
TaskTemplate.prototype.handleCommand_progress = async function (command) {
  ...
  let transaction = {
    if: [
      ...
      [ JSON.stringify('finished'), '!=', [ 'hget', `${prefix}${this.id}`, 'state' ] ]
    ],
    exec: [
  ...
  query.transaction(JSON.stringify(transaction));
...
TaskTemplate.prototype.handleCommand_error = async function (command) {
  ...
  let tasks = await this.__fillCancelInfo__();
  let cancelIDs = tasks.map(t => t.id).filter(id => id !== this.id);
  ...
  if (res[shift][1]) { // transaction result
...
TaskTemplate.prototype.__fillCancelInfo__ = async function () {
  return [ this ];
};
...
TaskTemplate.prototype.toObject = function () {
  return this.__serializableFields.reduce((obj, key) => {
...
TaskTemplate.prototype.fromObject = function (obj) {
  this.__serializableFields.forEach(key => {
