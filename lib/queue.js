...
class Queue extends EventEmitter {
  ...
  async __restartTask__(task, add_retry, delay) {
    ...
    let success = await this.__redis__.eval(
      this.__scripts__.transaction,
      1,
      JSON.stringify({
        validate: [
  ...
  async __collectChildren__(id, result) {
    ...
    let children = await this.__redis__.hget(`${this.__prefix__}${id}`, 'children');
  ...
  async __execTask__(id) {
    ...
    if (task.parent) {
      ...
      if (!parent || parent.state === 'finished') {
        ...
        let success = await this.__redis__.eval(this.__scripts__.transaction, 1, JSON.stringify(transaction));
    ...
    let locked = await this.__redis__.eval(
      this.__scripts__.transaction,
      1,
      JSON.stringify({
        validate: [
