...
for __, check in pairs(params["if"]) do
  if check[1] == "eq" then
    if check[2] ~= redis.call(unpack(check[3])) then return 0 end
  elseif check[1] == "ne" then
    if check[2] == redis.call(unpack(check[3])) then return 0 end
  elseif #check == 2 then
    -- alternate eq notation
...
class Queue extends EventEmitter {
  ...
  __init__() {
    ...
    this.__redis__.defineCommand('transaction', {
      lua: scripts.transaction,
      numberOfKeys: 0
    });
