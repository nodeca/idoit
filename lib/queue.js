...
function Queue({ redisURL, concurrency = 100, pool = 'default', ns = 'idoqueue:' } = {}) {
  ...
  this.__tick_progress__  = false; // true while `.__tick__()` executed
...
Queue.prototype.shutdown = function () {
  ...
  if (this.__tasksTracker__ === 0 && !this.__tick_progress__) return Promise.resolve(true);
...
//
Queue.prototype.__getRawTask__ = async function (id) {
...
//
Queue.prototype.__getRawTasks__ = async function (ids) {
...
//
Queue.prototype.getTask = async function (id) {
  // Unserialize payload
...
//
Queue.prototype.cancel = async function (id) {
...
//
Queue.prototype.__restartTask__ = async function (task, add_retry, delay) {
...
//
Queue.prototype.__collectChildren__ = async function (id, result) {
  if (!result) {
    result = [];
  } else {
    result.push(id);
...
//
Queue.prototype.__addTask__ = async function (task, delay) {
  let query        = this.__redis__.multi();
  let prefix       = this.__prefix__;
...
//
Queue.prototype.__execTask__ = async function (id) {
  let time = utils.redisToMs(await this.__redis__.timeAsync());
...
//
Queue.prototype.__consumeTasks__ = async function () {
...
//
Queue.prototype.__consumeCommands__ = async function () {
  let pools = Array.isArray(this.pool) ? this.pool : [ this.pool ];
...
//
//
Queue.prototype.__tick__ = async function () {
  if (this.__stopped__ || this.__tick_progress__) return;

  this.__tick_progress__ = true;
