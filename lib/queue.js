...
function Queue({ redisURL, concurrency = 100, pool = 'default', ns = 'idoqueue:' } = {}) {
...
//
Queue.prototype.__getRawTask__ = Promise.coroutine(function* (id) {
...
//
Queue.prototype.__getRawTasks__ = Promise.coroutine(function* (ids) {
  let query = this.__redis__.multi();

  ids.forEach(id => query.hgetall(`${this.__prefix__}${id}`));

  let serializedPayloads = yield query.execAsync();
...
Queue.prototype.getTask = Promise.coroutine(function* (id) {
  // Unserialize payload
  let payload = yield this.__getRawTask__(id);
...
Queue.prototype.cancel = Promise.coroutine(function* (id) {
  let task = yield this.__getRawTask__(id);
  ...
  //
  let rawTasks = yield this.__getRawTasks__(cancelIDs);
  ...
  cancelIDs = cancelIDs.filter((id, i) => rawTasks[i] && rawTasks[i].state !== 'finished');
...
Queue.prototype.__addTask__ = Promise.coroutine(function* (task, delay) {
  ...
  let previousTask = yield this.__getRawTask__(task.id);
...
Queue.prototype.__execTask__ = Promise.coroutine(function* (id) {
  ...
  //
  if (task.parent) {
    let parent = yield this.__getRawTask__(task.parent);
    ...
    if (parent.state === 'finished') {
      ...
      let success = yield this.__redis__.evalAsync(this.__scripts__.transaction, 1, JSON.stringify(transaction));
  ...
  if (error) {
    if (task.retries >= task.retry - 1) {
      ...
      if (success) {
        // Check that parent wasn't canceled (iterator doesn't cancel chunks)
        //
        // TODO: maybe fix iterator instead
        //
        let parentIsntCanceled = !task.parent || (yield this.getTask(task.parent)).state !== 'finished';

        /* eslint-disable max-depth */
        if (parentIsntCanceled) {
      ...
      let success = yield this.__redis__.evalAsync(
        this.__scripts__.transaction,
        1,
        JSON.stringify({
          validate: [
...
Queue.prototype.__consumeCommands__ = Promise.coroutine(function* () {
  ...
  for (let pool of pools) {
    ...
    for (let i = 0; i < commands.length; i += 2) {
      ...
      // - uid is invalid
      //
      yield this.__redis__.zremAsync(`${prefix_pool}commands_locked`, commands[i]);
    }
  }
});
