...
//
Queue.prototype.__restartTask__ = Promise.coroutine(function* (task, add_retry, delay) {
...
Queue.prototype.__execTask__ = Promise.coroutine(function* (id) {
  ...
  if (error) {
    if (task.retries >= task.retry - 1) {
      ...
      if (success) {
        let parentFinished = task.parent && (yield this.getTask(task.parent)).state === 'finished';
