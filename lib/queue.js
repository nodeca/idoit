...
function Queue({ redisURL, concurrency = 100, name = 'default', ns = 'idoqueue:' } = {}) {
  ...
  // Load lua scripts & patch `prefix` value
  fs.readdirSync(path.join(__dirname, '../scripts')).forEach(fileName => {
    let sname = path.basename(fileName, '.lua');
    let sbody = fs.readFileSync(path.join(__dirname, '../scripts', fileName), 'utf-8');
...
Queue.prototype.options = function (opts) {
  if (opts.concurrency) this.__concurrency__ = opts.concurrency;
  if (opts.name)        this.__pool_id__     = utils.poolToId(opts.name);
  if (opts.ns)          this.__prefix__      = opts.ns;
};
...
Queue.prototype.registerTask = function (...args) {
  ...
  if (options.poolName) {
    options.pool_id = utils.poolToId(options.poolName);
  }
  ...
  let TaskClass = options.baseClass.extend(options);
  ...
  this[options.name] = (...args) => {
    ...
    task.then = (onFulfilled, onRejected) => {
      if (!task.__promise__) {
        if (task.init) {
          task.__promise__ = Promise.resolve().then(() => task.init());
        } else {
          task.__promise__ = Promise.resolve();
        }
      }

      return task.__promise__.then(onFulfilled, onRejected);
    };
...
Queue.prototype.__execTask__ = Promise.coroutine(function* (id) {
  ...
  if (task.parent) {
    ...
    if (parent.state === 'finished') {
      ...
      yield this.__redis__.evalAsync(this.__scripts__.transaction, 1, JSON.stringify(transaction));
      return;
    }
  }
  ...
  let locked = yield this.__redis__.evalAsync(
    ...
    JSON.stringify({
      validate: [
        [ String(task.pool_id), [ 'zscore', `${prefix}startable`, task.id ] ]
      ],
      exec: [
        [ 'zrem', `${prefix}startable`, task.id ],
...
Queue.prototype.__consumeTasks__ = Promise.coroutine(function* () {
  ...
  let IDs = yield this.__redis__.zrangebyscoreAsync(
    `${this.__prefix__}startable`,
    this.__pool_id__,
    this.__pool_id__,
    'LIMIT',
    0,
...
Queue.prototype.__tick__ = Promise.coroutine(function* () {
  ...
  for (let i = 0; i < suspendedIDs.length; i += 2) {
    ...
    let pool_id = JSON.parse(yield this.__redis__.hgetAsync(`${prefix}${id}`, 'pool_id'));
    ...
    query.eval(
      ...
      JSON.stringify({
        ...
        exec: [
          ...
          [ 'zadd', `${prefix}startable`, pool_id, id ],
