...
function ChainTemplate(queue, ...args) {
  ...
  // If it's called like queue.task([ subtask1, subtask2, ...]).run(),
  // assume that arguments are an array of children for this group.
  //
  // Initialize children and mask args[0] to prevent it from being serialized.
  //
  // Otherwise, if it's called like queue.task(arg1, arg2, ...),
  // custom task `init` function should fill children list later.
  //
...
ChainTemplate.prototype.handleCommand_activate = Promise.coroutine(function* (command) {
  ...
  let rawChild = yield this.queue.__getRawTask__(this.children[0]);
...
ChainTemplate.prototype.handleCommand_result = Promise.coroutine(function* (command) {
  ...
  if (this.children_finished + 1 < this.children.length) {
    ...
    let rawChild = yield this.queue.__getRawTask__(childID);
